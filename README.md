...
| PUT    | /api/users/userID | If the user is logged in, and is the correct user responds with an object with the users entry. If the user is not logged-in or does not contain the entry respond with the err code. All values are neccessary.  | { "username": "username","password": "pass","firstname": "first", "lastname": "last" } |
